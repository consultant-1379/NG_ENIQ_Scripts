...
def no_status(projects,journey,journey_table_name):
	...
	if(f2 != 1):	
		HTMLT.write("<center><h3>No Analysis found for "+shipment+" branch in below projects</h3></center>")
		f2 = 1
	flag = 0
	for project in projects:
		if(flag != 1):
...
data_ingress = ['eric-oss-eniq-parser-3gpp32435','eric-oss-eniq-parser-ascii','eric-oss-eniq-parser-ct','eric-oss-eniq-parser-mdc','eric-oss-eniq-loadfile-builder','eric-oss-eniq-parser-common','eric-oss-eniq-loader-sapiq','eric-oss-eniq-parser-csexport','eric-oss-eniq-bulk-cm-handler']
...
data_calculator = ['eric-oss-eniq-aggregator','eric-oss-eniq-calculator','eric-oss-eniq-monitor','eric-oss-eniq-dc-ft-common']
tech_pack = ['eric-oss-eniq-feature-installer']
data_layer = ['eric-oss-eniq-cms-api','eric-oss-eniq-datawarehouse','eric-oss-eniq-iq-cert-manager','eric-oss-eniq-dwh-license-service']
...
HTMLT.write("<html>\n<head><center><h2>cEniq SonarQube Analysis Report</h2></center></head>\n<body>")
HTMLT.close()
...
no_branch1 = data_calculator
final_dict = {}
for i in range(length):		
	no_branch = []
	yes_branch = []

	l = check_branch(no_branch1,"datacalculator",l1[i].strip())

	no_branch1 = []
	no_branch1 = no_branch

create_table(final_dict,"datacalculator","DataCalculator")

###For Techpack
no_branch = []
yes_branch = []

no_branch1 = tech_pack
final_dict = {}
for i in range(length):
	no_branch = []
	yes_branch = []

	l = check_branch(no_branch1,"techpack",l1[i].strip())

	no_branch1 = []
	no_branch1 = no_branch

create_table(final_dict,"techpack","Techpack")

###For Datalayer
no_branch = []
yes_branch = []

no_branch1 = data_layer
final_dict = {}
for i in range(length):
	no_branch = []
	yes_branch = []

	l = check_branch(no_branch1,"datalayer",l1[i].strip())

	no_branch1 = []
	no_branch1 = no_branch

create_table(final_dict,"datalayer","DataLayer")
